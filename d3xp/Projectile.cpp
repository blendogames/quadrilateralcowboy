...
void idProjectile::Spawn( void ) {
	...
	if (spawnArgs.GetString("md5model"))
	{
		idDict args;
		args.Clear();
		args.SetVector(	"origin",   this->GetPhysics()->GetOrigin() );
		args.Set( "model",			spawnArgs.GetString("md5model"));
		md5model = ( idAnimatedEntity * )gameLocal.SpawnEntityType( idAnimatedEntity::Type, &args );
		md5model->SetAxis( this->GetPhysics()->GetAxis() );
		md5model->Bind(this, true);
	}
}
...
void idProjectile::Launch( const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity, const float timeSinceFire, const float launchPower, const float dmgPower ) {
	...
	if ( spawnArgs.GetBool( "tracers" )  )  //BC always do tracers.
	{
	...
	if ( !gameLocal.isClient ) {
		if ( fuse <= 0 ) {
			...
			if (md5model)
			{
				md5model->PostEventSec( &EV_Remove, fuse );
			}
		}
		else
		{
	...
	if ( projectileFlags.isTracer ) {
		...
		StartSound( "snd_fly", SND_CHANNEL_BODY2, 0, false, NULL );
	}
...
void idProjectile::Think( void ) {
	...
	// add the particles
	if ( smokeFly != NULL && smokeFlyTime && !IsHidden() && stickyState <= 0)
	{
...
bool idProjectile::Collide( const trace_t &collision, const idVec3 &velocity ) {
	...
	//BC 4-15-2016 make player drop held items.
	//BC 7-16-2016 only drop item if projectile has push ability.
	ent = gameLocal.entities[ collision.c.entityNum ];
	if (gameLocal.GetLocalPlayer()->pickerWeapon.dragEnt.IsValid() && ent->IsType( idPlayer::Type ) && spawnArgs.GetInt("pushamount") > 0)
	{
		idEntity *dragee;
	...
	if ( gameLocal.isClient ) {
		if ( ClientPredictionCollide( this, spawnArgs, collision, velocity, !spawnArgs.GetBool( "net_instanthit" ) ) )
		{
			...
			//bc 2-15-2016 possibly hit player that is attached to deck.
			//knock player off deck.
			ent = gameLocal.entities[ collision.c.entityNum ];
	...
	else if (spawnArgs.GetBool("sticky"))
	{
		//Sticky projectile has collided into world.

		idDict args;
		args.Clear();
		args.SetVector( "origin",		this->GetPhysics()->GetOrigin());
		args.Set(		"model",		spawnArgs.GetString("smoke_stickyhit") );
		...
		if (ent->IsType( idWorldspawn::Type ) || ent->IsType( idPlayer::Type ))
		{
			stickyState = 1;
		}
		else
		...
		float stickyfuse = spawnArgs.GetFloat("stickyfuse");
		PostEventSec( &EV_Explode, stickyfuse );

		if (md5model)
		{
			md5model->PostEventSec( &EV_Remove, stickyfuse );

			int anim;
			int channel = 0;
			int animBlendFrames = 4;

			anim = md5model->GetAnimator()->GetAnim( spawnArgs.GetString("stickyanim") );
			if (anim)
			{
				md5model->GetAnimator()->PlayAnim( channel, anim, gameLocal.time, FRAME2MS( animBlendFrames ) );
			}
		}

		StopSound( SND_CHANNEL_ANY, false );

		StartSound( "snd_stickyhit", SND_CHANNEL_BODY2, 0, false, NULL );
		StartSound( "snd_stickyidle", SND_CHANNEL_BODY3, 0, false, NULL );
		

		const char *sticky_damage = spawnArgs.GetString( "def_sticky_damage" );
		gameLocal.RadiusDamage( physicsObj.GetOrigin(), this, owner.GetEntity(), NULL, this, sticky_damage, damagePower );
...
void idProjectile::Explode( const trace_t &collision, idEntity *ignore ) {
	...
	if (gameLocal.entities[collision.c.entityNum]->IsType(idMoveableItem::Type) || gameLocal.entities[collision.c.entityNum]->IsType(idMoveable::Type))
	{
		idDict args;
		args.Clear();
		args.SetVector( "origin",		collision.endpos);
		args.Set(		"model",		spawnArgs.GetString("model_hitmoveable", "sparkburst6.prt") );
		args.SetBool(	"start_off",	false );
		gameLocal.SpawnEntityType( idExplodable::Type, &args );
	...
	if (spawnArgs.GetBool("fractures", "0"))
	{
		if (gameLocal.entities[ collision.c.entityNum ]->IsType( idBrittleFracture::Type ) || gameLocal.entities[ collision.c.entityNum ]->IsType( idQGlass::Type ))
		{
			gameLocal.entities[ collision.c.entityNum ]->AddForce(this, this->entityNumber, collision.endpos, collision.c.normal);

			
			//BC
			//projectiles penetrate multiple levels of fracture glass.
			
			idVec3 movedir = physicsObj.GetOrigin() - startOrigin;
			movedir.Normalize();

			int i;
			idVec3 hitpoint = physicsObj.GetOrigin();
			...
			for (i = 0; i < maxGlassPanes; i++)
			{
				...
				gameLocal.clip.TracePoint( tr, startTrace, startTrace + (movedir * 4096), MASK_SOLID | MASK_SHOT_RENDERMODEL, this );
				
				
				if (developer.GetBool())
					gameRenderWorld->DebugArrow( colorRed, startTrace, tr.endpos, 4, 30000 );
				...
				if ( hitent != NULL )
				{
					...
					//common->Printf("hit ent %s\n", hitent->name.c_str());


					if (gameLocal.entities[ tr.c.entityNum ]->IsType( idBrittleFracture::Type ) || gameLocal.entities[ tr.c.entityNum ]->IsType( idQGlass::Type ))
					{
						//shatter it.
						gameLocal.entities[ tr.c.entityNum ]->AddForce(this, this->entityNumber, tr.endpos, tr.c.normal);
						hitpoint = tr.endpos;
						continue;
					}
					else if (gameLocal.entities[ tr.c.entityNum ]->IsType( idTrembler::Type ))
					{
						static_cast< idTrembler * >( gameLocal.entities[ tr.c.entityNum ] )->Event_Touch(NULL, NULL);
					}
				}
	...
	idEntity *markerEnt = gameLocal.FindEntity( spawnArgs.GetString( "markername" ) );
	if (markerEnt)
	{
		markerEnt->PostEventMS( &EV_Remove, 0);
	}

	idEntity *beamEnt = gameLocal.FindEntity( spawnArgs.GetString( "beamname" ) );
	if (beamEnt)
	{
		beamEnt->PostEventMS( &EV_Remove, 0);
	}
...
void idProjectile::HandleHit(const trace_t &collision)
{
	if (gameLocal.entities[ collision.c.entityNum ])
	{
		if (gameLocal.entities[ collision.c.entityNum ]->IsType(idFrobCube::Type) || gameLocal.entities[ collision.c.entityNum ]->IsType(idStaticEntity::Type))
		{
			idStr scriptName = gameLocal.entities[ collision.c.entityNum ]->spawnArgs.GetString( "calldamage" );
	...
	//BC
	//frob stuff.
	if (spawnArgs.GetBool( "frobber" ))
	{
		if (ENTITYNUM_WORLD != collision.c.entityNum && collision.c.entityNum >= 0)
		{
			if (gameLocal.entities[ collision.c.entityNum ])
			{

				//common->Printf("Hit object: %s\n", gameLocal.entities[ collision.c.entityNum ]->GetName());

				if (gameLocal.entities[ collision.c.entityNum ]->spawnArgs.GetInt("sentryhittable") )
				{
					gameLocal.GetLocalPlayer()->DoFrob(gameLocal.entities[ collision.c.entityNum ]);


					idDict args;
					args.Clear();
					args.SetVector( "origin",		collision.endpos);
					args.Set(		"model",		"sparkburst5.prt" );
					args.SetBool(	"start_off",	false );
					idEntity *zapParticles = gameLocal.SpawnEntityType( idExplodable::Type, &args );
					idAngles ang = this->GetPhysics()->GetAxis().ToAngles();
					ang.yaw += 180;
					zapParticles->SetAngles(ang);
				}
			}
		}
	...
	//bc push physical things.
	int pushAmount = spawnArgs.GetInt( "pushamount" );
	if (pushAmount > 0)
	{
		if (ENTITYNUM_WORLD != collision.c.entityNum && collision.c.entityNum >= 0)
		{
			...
			{
				//common->Printf("hit %s\n", gameLocal.entities[ collision.c.entityNum ]->GetName());
				if (gameLocal.entities[ collision.c.entityNum ]->IsType( idMoveable::Type ) || gameLocal.entities[ collision.c.entityNum ]->IsType( idMoveableItem::Type))
				{
					//BC 9-10-2015 if player is holding item, then make player drop the item.
					if (gameLocal.GetLocalPlayer()->pickerWeapon.dragEnt.IsValid())
					{
						if (gameLocal.GetLocalPlayer()->pickerWeapon.dragEnt.GetEntity()->IsGrabbed())
						{
							if (gameLocal.GetLocalPlayer()->pickerWeapon.dragEnt.GetEntity()->entityNumber == collision.c.entityNum)
							{
								//drop the item.
								gameLocal.GetLocalPlayer()->pickerWeapon.StopDrag(true, false);
							}
					...
					gameLocal.entities[ collision.c.entityNum ]->GetPhysics()->SetLinearVelocity(facing * pushAmount);
				}
			}
		}
	}
