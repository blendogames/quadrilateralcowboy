...
void idPhysics_RigidBody::Integrate( float deltaTime, rigidBodyPState_t &next ) {
	...
	{
		if (gameLocal.GetLocalPlayer()->pickerWeapon.dragEnt.GetEntityNum() == self->entityNumber)
		{
			shouldGravity = false;
		}
	}
	...
	//BC water code.
	/*
	int testC = gameLocal.clip.Contents( this->GetOrigin(), NULL, mat3_identity, CONTENTS_WATER, NULL );
	if ( testC & CONTENTS_WATER )
	{
		//water gravity.
		next.i.linearMomentum += deltaTime * idVec3(0,0,0) * mass;
	}
	else
	{
		//normal gravity.
...
void idPhysics_RigidBody::DebugDraw( void ) {
	...
	if ( rb_showBodies.GetBool() || ( rb_showActive.GetBool() && current.atRest < 0 ) ) {
		collisionModelManager->DrawModel( clipModel->Handle(), clipModel->GetOrigin(), clipModel->GetAxis(), vec3_origin, 0.0f, idVec4(0,1,0,1) );
	}
