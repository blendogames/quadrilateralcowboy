...
bool idPhysics_Parametric::TestIfAtRest( void ) const {
	...
	//for the pneumo basket. call function 
	

	const char *pneumoMaster = self->spawnArgs.GetString("onsplineend");
	if (pneumoMaster)
	{
		//have a name. try to find the ent.
		idEntity *ent;
		ent = gameLocal.FindEntity( pneumoMaster );

		if (ent)
		{
			gameLocal.GetLocalPlayer()->UseFrob( ent, "onSplineEnd" );

			if (ent->IsType(idPowerSawGeneric::Type))
			{
				//frob the powersaw generic.
				idPowerSawGeneric *gate = static_cast<idPowerSawGeneric *>( ent );
				gate->OnSplineEnd();
			}
		}
	}
...
int idPhysics_Parametric::GetLinearEndTime( void ) const {
	...
	{
		//float val = current.linearExtrapolation.GetEndTime();		
		//gameLocal.Printf("endtime: %f\n", val);
		//return val;
