...
void idTarget_EndLevel::Event_Activate( idEntity *activator ) {
	...
	//bc 3-30-2016 only trigger this if it's an interstitial level.
	if (gameLocal.GetMapInterstitial())
	{
		idStr levelIndex = gameLocal.GetMapIndex();
		cvarSystem->SetCVarBool( va("g_leveldone_%s", levelIndex.c_str()), true);
	}
	...
	if ( spawnArgs.GetBool( "endOfGame" ) ) {
		cvarSystem->SetCVarBool( "g_nightmare", true );
		gameLocal.sessionCommand = "endgamedisconnect";
		return;
	}
#endif
...
void idTarget_SetFov::Event_Activate( idEntity *activator ) {
	...
	if (spawnArgs.GetBool("reverse")) //start at ent fov, then transition to default fov.
	{
		fovSetting.Init( gameLocal.time, SEC2MS( spawnArgs.GetFloat( "time" ) ), spawnArgs.GetFloat( "fov" ), player ? player->DefaultFov() : g_fov.GetFloat() );
	}
	else	
	{
