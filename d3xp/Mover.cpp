...
CLASS_DECLARATION( idEntity, idMover )
	...
	EVENT( EV_GetOriginalPosition,	idMover::Event_GetOriginalPosition )
	EVENT( EV_GetOriginalAngle,		idMover::Event_GetOriginalAngle )
END_CLASS
...
void idMover::UpdateRotationSound( moveStage_t stage ) {
	switch( stage ) {
		...
		case FINISHED_STAGE: {
			...
			if (idStr::Icmp( this->spawnArgs.GetString("classname"), "func_rotdoor" ) == 0)
			{
				idRotDoor *rotdoor = static_cast<idRotDoor *>( this );

				if (rotdoor)
				{
					if ( rotdoor->shouldOpen )
					{
...
void idMover::Event_GetOriginalPosition( void )
{
	idThread::ReturnVector(this->originalPosition);
}

void idMover::Event_GetOriginalAngle( void )
{
	idAngles ang = this->originalAngle.ToAngles();
...
================
*/
bool idMover::Event_IsRotating( void )
{
	...
	if ( moveMaster == this ) {
		switch( state ) {
			...
			case MOVER_1TO2:
				...
				StartSound( "snd_move", SND_CHANNEL_RADIO, 0, false, NULL );
...
void idMover_Binary::GotoPosition2( void ) {
	...
	if ( moverState == MOVER_2TO1 ) {
		// use the physics times because this might be executed during the physics simulation
		partial = physicsObj.GetLinearEndTime() - physicsObj.GetTime();
		assert( partial >= 0 ); //BC todo: crash here, partial = -16  (why???)
		if ( partial < 0 ) {
			partial = 0;
		}
...
const idEventDef EV_Door_OnSoftAlarm( "<OnSoftAlarm>", NULL );
...
CLASS_DECLARATION( idMover_Binary, idDoor )
	...
	EVENT( EV_Door_OnSoftAlarm,			idDoor::Event_OnSoftAlarm)
...
void idDoor::Event_OnSoftAlarm( void )
{
	if (this->IsOpen() && this->IsLocked() && !this->softAlarmActivated)
	{
...
void idDoor::Event_TeamBlocked( idEntity *blockedEntity, idEntity *blockingEntity ) {
	...
	if (spawnArgs.GetInt( "playblocksound" ))
		StartSound( "snd_blocked", SND_CHANNEL_ANY, 0, false, NULL );

	//if door is locked, then display the blocked-door message.
	if (spawnArgs.GetInt( "locked" ) && spawnArgs.GetInt( "playblocksound" ))
	{
		gameLocal.GetLocalPlayer()->Event_hudMessage( va( common->GetLanguageDict()->GetString( "#str_02117" ), this->GetName() ) );
	}

	if (spawnArgs.GetInt( "locked" ))
	{
		//BC
		//Since doors are set to never auto-close, a blocked door will remain open forever.
		//Therefore, after getting blocked, force the door to close after XXX amount of time.
		PostEventSec( &EV_Mover_ReturnToPos1, 1 );
	}
...
void idDoor::Event_Touch( idEntity *other, trace_t *trace ) {
	...
	{
		if ( other && other->IsType( idPlayer::Type ) && IsLocked() && !IsNoTouch() )
		{
			PlayLockedSound();
		}
	}
...
void idDoor::PlayLockedSound( void )
{
...
void idDoor::Event_Activate( idEntity *activator ) {
	...
	if ( spawnArgs.GetInt( "locked" ) ) {
		...
		//Lock( 0 );  //bc don't auto unlock doors
		if ( old_lock == 2 ) {
			return;
		}
	}
...
{
	if (spawnArgs.GetInt("unhackable") >= 1)
	{
		return;
	}

	float softalarmtime = spawnArgs.GetFloat( "softalarmtime", "3" );

	//BC
	if (this->IsLocked() && this->softAlarmLasttime < gameLocal.time && softalarmtime > 0)
	{
		uiHackTimer = gameLocal.time + UIHACKTIME;

		this->softAlarmActivated = false;
		this->softAlarmLasttime = gameLocal.time;

		float softAlarmDelay = 600 /*leeway time*/ + (softalarmtime * 1000.0f);
		PostEventMS( &EV_Door_OnSoftAlarm, softAlarmDelay );
	}
