...
void idWeapon::Restore( idRestoreGame *savefile ) {
	...
	WEAPON_PICKER.LinkTo(		scriptObject, "WEAPON_PICKER" );
...
void idWeapon::Clear( void ) {
	...
	weaponAngleOffsetScale		= 0.25f;
	weaponAngleOffsetMax		= 10.0f;
	weaponOffsetTime			= 1000.0f;
	weaponOffsetScale			= 0.001f;
...
void idWeapon::GetWeaponDef( const char *objectname, int ammoinclip ) {
	...
	placerModel			= weaponDef->dict.GetString( "model" );
	...
	if ( ( ammoClip < 0 ) || ( ammoClip > clipSize ) ) {
		...
		//bc remove this auto clip behavior.
		/*
#ifdef _D3XP
...
*/
	...
	zoomFov = weaponDef->dict.GetInt( "zoomFov", "30" ); //default zoom amount.
	...
	weaponOffsetTime = weaponDef->dict.GetFloat( "weaponOffsetTime", "1000" );
	weaponOffsetScale = weaponDef->dict.GetFloat( "weaponOffsetScale", "0.001" );
...
void idWeapon::UpdateGUI( void ) {
	...
	renderEntity.gui[ 0 ]->SetStateString( "text_objective", va("%s", owner->text_objective.c_str()));	
	renderEntity.gui[ 0 ]->SetStateString( "text_alarms", va("%d", owner->text_alarms));


	int minutes = owner->text_missionclock / 60.0f;
	float seconds = owner->text_missionclock - (minutes * 60);
	int deciseconds = (seconds - (int)seconds) * 10.0f;
	
	renderEntity.gui[ 0 ]->SetStateString( "text_missionclock", va("%d:%02d.%d",  minutes, (int)(seconds), deciseconds   ));
}
