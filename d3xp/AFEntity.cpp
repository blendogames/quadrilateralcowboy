...
#define STEER_MAXANGLE		35
#define STEER_MAXANGLE_AI	55

#define	HIT_WAYPOINT_THRESHOLD	200
...
const idEventDef EV_SetAFGravity( "setAFGravity", "v" );
const idEventDef EV_SetAFActive( "setAFActive", "d" );
const idEventDef EV_SetAFRest( "setAFRest", "d" );
...
CLASS_DECLARATION( idAnimatedEntity, idAFEntity_Base )
	...
	EVENT( EV_SetAFGravity,	idAFEntity_Base::Event_SetAFGravity)
	EVENT( EV_SetAFActive,	idAFEntity_Base::Event_SetAFActive)
	EVENT( EV_SetAFRest,	idAFEntity_Base::Event_SetAFRest)
END_CLASS
...
void idAFEntity_Base::Event_SetAFGravity( const idVec3 &newGravity )
{
	af.GetPhysics()->SetGravity( newGravity );
}

void idAFEntity_Base::Event_SetAFRest( int value )
{
	this->af.GetPhysics()->SetComeToRest( value );

	if ( this->IsType( idAFEntity_VehicleSimple::Type ) )
	{
		int i;
		idAFEntity_VehicleSimple *	focusVehicle;
		focusVehicle = static_cast<idAFEntity_VehicleSimple *>( this );

		focusVehicle->stoppingState = 1;
	}	
}

void idAFEntity_Base::Event_SetAFActive( int value )
{
	if (value <= 0)
		this->af.Rest();
	else
...
void idAFEntity_Vehicle::Spawn( void ) {
	...
	//BC where player is bound to. we do this so the player viewangle doesnt rotate when the car rotates.
	idDict args;

	idVec3 bindOrigin;
	idMat3 bindAxis;
	animator.GetJointTransform( eyesJoint, gameLocal.time, bindOrigin, bindAxis );
	bindOrigin = renderEntity.origin + bindOrigin * renderEntity.axis;

	args.Clear();
	args.SetVector( "origin", bindOrigin );
	args.Set( "classname", "info_location" );
	args.Set( "solid", "0" );
	args.SetInt( "thirdperson", 1);
	gameLocal.SpawnEntityDef( args, &this->bindPoint );

	this->bindPoint->Bind(this, false);
}
...
void idAFEntity_Vehicle::Use( idPlayer *other ) {
	...
	else
	{
		...
		//set angle.
		player->SetViewAngles( renderEntity.axis.ToAngles() );

		//bind.
		//player->BindToBody( this, 0, true );
		player->Bind(this->bindPoint, false);
...
float idAFEntity_Vehicle::GetSteerAngle( void )
{
	...
	if (g_vehicleControlType.GetInteger() <= 0)
	{
		float cameraYaw = player->viewAngles.yaw;
		float buggyYaw = this->renderEntity.axis.ToAngles().yaw;
		cameraYaw = idMath::AngleNormalize360(cameraYaw);
		buggyYaw = idMath::AngleNormalize360(buggyYaw);
		...
		{
			float delta_yaw;
			delta_yaw = buggyYaw - cameraYaw;

			if (idMath::Fabs( delta_yaw ) > 180.0f )
			{
				if (delta_yaw > 0)
				{
			...
			delta_yaw = idMath::ClampFloat( -STEER_MAXANGLE, STEER_MAXANGLE, delta_yaw );

			if (player->usercmd.forwardmove < 0)
				delta_yaw = -delta_yaw;

			idealSteerAngle = delta_yaw;
		}
	}
	else
	{
	...
	if (idMath::Fabs( steerAngle) <= 1.0f)
	{
		steerAngle = 0.0f;
	}

		//gameLocal.GetLocalPlayer()->DebugMessage(va("ang %f\n", steerAngle));
...
const idEventDef EV_Vehicle_isLightsOff( "isLightsOff", NULL, 'd' );

CLASS_DECLARATION( idAFEntity_Vehicle, idAFEntity_VehicleSimple )
	EVENT( EV_Vehicle_isLightsOff,		idAFEntity_VehicleSimple::Event_isLightsOff )
END_CLASS
...
{
	//bc todo: fix this
	savefile->WriteInt(stoppingState);
	savefile->WriteInt(ignitionEndtime);
	savefile->WriteBool(isAI);
	savefile->WriteBool(attackHeld);
	savefile->WriteBool(gasHeld);
	savefile->WriteClipModel(wheelModel);

	int i;
	for (i = 0; i < 4; i++)
	{
		savefile->WriteFloat(wheelAngles[i]);
		savefile->WriteJoint(wheelJoints[i]);


		//BC todo: fix vehicle saving.
		suspension[i]->Save(savefile);
	}
}
...
{
	//bc todo: fix this
	savefile->ReadInt(stoppingState);
	savefile->ReadInt(ignitionEndtime);
	savefile->ReadBool(isAI);
	savefile->ReadBool(attackHeld);
	savefile->ReadBool(gasHeld);
	savefile->ReadClipModel(wheelModel);

	int i;
	for (i = 0; i < 4; i++)
	{
		savefile->ReadFloat(wheelAngles[i]);
		savefile->ReadJoint(wheelJoints[i]);
		suspension[i] = NULL; //BC todo: fix save/load on this vehicle.

		

		suspension[i] = static_cast<idAFConstraint_Suspension *>( af.GetPhysics()->GetConstraint( va("suspension%d", i) ) );
		//suspension[i] = static_cast<idAFConstraint_Suspension *>( suspension[i] );
		suspension[i]->Restore( savefile );
	}
}
...
void idAFEntity_VehicleSimple::SpawnSuspension( void )
{

}

void idAFEntity_VehicleSimple::Event_isLightsOff( void )
{
	//if engine is running then vehicle is definitely NOT hiding.
	if (this->stoppingState <= 0)
	{
...
void idAFEntity_VehicleSimple::Think( void ) {
	...
	if ( thinkFlags & TH_THINK )
	{
		...
		if ( player )
		{
			...
			if (g_vehicleControlType.GetInteger() <= 0)
			{
				idAFBody *body1 = af.GetPhysics()->GetBody( 0 );
				float curspeed = body1->GetPointVelocity( this->GetPhysics()->GetOrigin() ) *  body1->GetWorldAxis()[0];

				if (player->usercmd.forwardmove > 0 && curspeed < -15)
					steerAngle = 0; //if accelerating while moving backwards, then just move straight.
				else if (player->usercmd.forwardmove < 0 && curspeed > 15)
					steerAngle = 0; //if braking while moving forward, then just move straight.
			}
			...
			if (player->usercmd.forwardmove < 0)
			{
				this->renderEntity.shaderParms[ 7 ] = 1.0f;
			}
			else
			{
			...
			//attack.
			if ((player->usercmd.buttons & BUTTON_ATTACK) && !attackHeld && this->stoppingState == 0)
			{
				attackHeld = true;
				player->UseFrob(this, "onTurnoff");				
			}
			...
			if (player->usercmd.forwardmove != 0)
			{
				if (this->stoppingState == 0 && !this->gasHeld)
				{
					this->gasHeld = true;
					this->StartSound(  "snd_cargas" , SND_CHANNEL_BODY, 0, false, NULL );
					this->StopSound(SND_CHANNEL_VOICE2, false);
				}
			}
			else if (this->gasHeld)
			{				
				this->gasHeld = false;

				if (this->stoppingState == 0)
				{
					this->StartSound(  "snd_caridle" , SND_CHANNEL_VOICE2, 0, false, NULL ); //start idle noise.
				}

				this->StopSound(SND_CHANNEL_BODY, false); //stop gas noise.
				this->StartSound(  "snd_cargasend" , SND_CHANNEL_BODY, 0, false, NULL ); //start idle noise.
			}
		}
		else if (this->isAI)
		{
			velocity = g_vehicleVelocity.GetFloat();
			velocity *= 1.1f;
			force = idMath::Fabs( 127 * g_vehicleForce.GetFloat() ) * (1.0f / 128.0f);
			force *= 1.2f;
		}

		if (this->stoppingState == 1)
		{
			idAFBody *body1 = af.GetPhysics()->GetBody( 0 );
			float curspeed = body1->GetPointVelocity( this->GetPhysics()->GetOrigin() ) *  body1->GetWorldAxis()[0];

			float force = idMath::Fabs( 127 * g_vehicleForce.GetFloat() ) * (1.0f / 128.0f);

			for ( i = 0; i < 4; i++ )
			{
				this->suspension[i]->EnableMotor( true );
				this->suspension[i]->SetMotorVelocity( -curspeed  );
				this->suspension[i]->SetMotorForce( force  );
			}


			if (idMath::Fabs( curspeed ) <= 2)
			{
				this->stoppingState = 2;
				this->af.Rest();
			}
		}
		else if (this->stoppingState == 2)
		{
			if (player)
			{
				if (player->usercmd.forwardmove != 0 || (player->usercmd.buttons & BUTTON_ATTACK && !this->attackHeld))
				{
					//turn car back on.
					if (player->usercmd.buttons & BUTTON_ATTACK)
						this->attackHeld = true;
					
					this->StartSound(  "snd_carignition", SND_CHANNEL_VOICE, 0, false, NULL );

					this->ignitionEndtime = gameLocal.time  + 700;
					this->stoppingState = 3;
				}
			}			
		}
		else if (this->stoppingState == 3)
		{
			if (ignitionEndtime <= gameLocal.time )
			{
				this->af.Start();
				...
				this->stoppingState = 0;
				player->UseFrob( this, "onTurnon" );
			}
		}
		else
		{

			//gameLocal.GetLocalPlayer()->DebugMessage(va("vel %f  %f\n", velocity, force));
		...
		rotation.SetVec( axis[1] );
		rotation.SetAngle( steerAngle * 1.5f );
		...
		if ( force != 0.0f && !( gameLocal.framenum & 7 ) )
		{
			...
			for ( i = 0; i < 4; i++ )
			{
				...
				{
					gameLocal.smokeParticles->EmitSmoke( dustSmoke, gameLocal.time, gameLocal.random.RandomFloat(), contacts[j]->GetContact().point, contacts[j]->GetContact().normal.ToMat3(), 0 );
				}
			}
		}
		*/
		...
		//bc wheelsmoke
		if ( force != 0.0f && !( gameLocal.framenum & 3 ) && this->stoppingState == 0)
		{
			for ( i = 2; i < 4; i++ )
			{
				idVec3 wheelorigin = suspension[i]->GetWheelOrigin();
				wheelorigin[2] -= this->wheelRadius * 0.8f;
				gameLocal.smokeParticles->EmitSmoke( dustSmoke, gameLocal.time, gameLocal.random.RandomFloat(),
					wheelorigin,
					this->renderEntity.axis,
					0 );
			}
		}
	}
...
CLASS_DECLARATION( idAFEntity_VehicleSimple, idAFEntity_VehicleAutomated )
...
{
	savefile->WriteFloat(steeringSpeed);
	savefile->WriteFloat(currentSteering);
	savefile->WriteFloat(idealSteering);
	savefile->WriteFloat(originHeight);

	savefile->WriteObject(waypoint);
}

void idAFEntity_VehicleAutomated::Restore( idRestoreGame *savefile )
{
	savefile->ReadFloat(steeringSpeed);
	savefile->ReadFloat(currentSteering);
	savefile->ReadFloat(idealSteering);
	savefile->ReadFloat(originHeight);

	savefile->ReadObject(reinterpret_cast<idClass *&>(waypoint));
}
...
void idAFEntity_VehicleAutomated::Think( void ) {
	...
	delta_yaw = idMath::ClampFloat( -STEER_MAXANGLE_AI, STEER_MAXANGLE_AI, delta_yaw );
