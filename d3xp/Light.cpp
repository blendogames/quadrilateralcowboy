...
const idEventDef EV_Light_FadeLight( "fadeLight", "fv" );

//bc
const idEventDef EV_Light_IsOn( "IsOn", NULL, 'd' );
const idEventDef EV_Light_Reset( "resetlight", "f" );
...
CLASS_DECLARATION( idEntity, idLight )
	...
	EVENT( EV_Light_FadeLight,		idLight::Event_FadeLight )
	EVENT( EV_Light_IsOn,			idLight::Event_IsOn )
	EVENT( EV_Light_Reset,			idLight::Event_Reset )
END_CLASS
...
void idLight::Save( idSaveGame *savefile ) const {
	...
	savefile->WriteVec3( originalColor );
}
...
void idLight::Restore( idRestoreGame *savefile ) {
	...
	savefile->ReadVec3( originalColor );
...
void idLight::Spawn( void ) {
	...
	originalColor.Set( renderLight.shaderParms[ SHADERPARM_RED ], renderLight.shaderParms[ SHADERPARM_GREEN ], renderLight.shaderParms[ SHADERPARM_BLUE ] );
...
void idLight::Event_Reset( float time )
{
	Event_FadeLight(time, this->originalColor);
}

void idLight::Event_IsOn( void )
{
	if (this->isOn)
	{
