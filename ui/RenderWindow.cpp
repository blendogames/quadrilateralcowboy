...
void idRenderWindow::PreRender() {
	if (needsRender) {
		...
		spawnArgs.Set("skin", skinName); //bc
...
void idRenderWindow::Render( int time ) {
	...
	if ( worldEntity.hModel ) {
		...
		//BC handle the envirosuit meter rotation.
		worldEntity.shaderParms[4] = this->rotate;
...
void idRenderWindow::Draw(int time, float x, float y) {
	...
	refdef.fov_x = 90;
	refdef.fov_y = 1.5f * atan((float)drawRect.h / drawRect.w) * idMath::M_RAD2DEG; //bc was 2
