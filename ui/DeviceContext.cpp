...
void idDeviceContext::DrawEditCursor( float x, float y, float scale ) {
	...
	SetFontByScale(scale);
	float useScale = scale * useFont->glyphScale;
	const glyphInfo_t *glyph2;
	
	if (overStrikeMode)
		glyph2 = &useFont->glyphs['_'];
	else
		glyph2 = &useFont->glyphs[1];

	//&useFont->glyphs[(overStrikeMode) ? '|' : '_'];

	float	yadj = useScale * glyph2->top;

	/*
	glyph2 = &useFont->glyphs[1];
	//bc shift the pipe character so it's not directly overlapping the text.
	if (overStrikeMode)
		x -= (glyph2->imageWidth / 2) - 1;*/
