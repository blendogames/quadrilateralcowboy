...
void idSliderWindow::UpdateCvar( bool read, bool force ) {
	...
	//bc
	gui->SetStateInt( va( "%s_value", name.c_str() ), (value + modifiedSum) * modifiedMultiplier );

	gui->SetStateString( va( "%s_valueFloat", name.c_str() ), va( "%.1f", (value + modifiedSum) * modifiedMultiplier ) );
}
