...
bool idUserInterfaceLocal::WriteToSaveGame( idFile *savefile ) const {
	...
	for( int i = 0; i < num; i++ ) {
		...
		//BC 7-26-2016 do not save deck output.
		if (!idStr::Cmpn( string, "deckoutput_item_", 16 ))
		{
			//write dummy data.

			idStr dummyString = "z";
			int dummyLen = dummyString.Length();

			savefile->Write( &dummyLen, sizeof(dummyLen) );
			savefile->Write( dummyString.c_str(), dummyLen );
