...
void idSessionLocal::SetSaveGameGuiVars( void ) {
	...
	for ( i = 0; i < fileList.Num(); i++ ) {
		...
		//bc 9-1-2015 show date as "today" "yesterday" "2 days ago" etc.

		//first extract the time string.
		idStr		timeofday;
		int			timetab;
		time_t		today;
		...
		timetab = date.Find( '\t' );
		timeofday = date.Right( date.Length() - timetab - 1);
		timeofday.StripTrailingWhitespace(); //remove whitespace.
		timeofday.StripLeading( ' ' );		//remove whitespace.
		timeofday.ToUpper();					//force lower case.
		...
		name += va("%s\t%s", diffString.c_str(), timeofday.c_str());
		//END OF SHOWDATE CODE.
...
void idSessionLocal::SetModsMenuGuiVars( void ) {
	...
	if (game->GetSteamInitialized())
	{
		...
		int workshopTotal = game->GetWorkshopAmount();
		common->Printf("STEAM: subscribed workshop items: %d\n", workshopTotal);

		for (i = 0; i < workshopTotal; i++)
		{
			idStr path = game->GetWorkshopPathAtIndex(i);
		...
		common->Printf("STEAM: installed workshop items:  %d\n", workshopInstalledAmount);
	...
	{
		idStr modName;
		modName = list->GetDescription( i );
		modName += "\t";
		modName += "Local";
		guiActive->SetStateString( va("modsList_item_%i", i), modName.c_str() );
		
		common->Printf("MOD: local mod path: %s\n", list->GetMod( i ));
	...
	if (game->GetSteamInitialized())
	{
		for ( i = 0; i < workshopInstalledAmount; i++ )
		{
			...
			// TODO: Add support for adding full game paths.  I almost have this.  Just need to solve
			// loading up the level.script
#if 1
#ifdef _WIN32
#define PATH_SLASH "\\"
#else
...
#undef PATH_SLASH
			...
			modName += "\t";
			modName += "Workshop";
			guiActive->SetStateString( va("modsList_item_%i", i + list->GetNumMods()), modName.c_str() );
		...
		{
			common->Printf("STEAM: mod path %d: %s\n", i, modsList[i].c_str());
		}
	}

#endif

	guiActive->DeleteStateVar( va("modsList_item_%i", totalNumberOfMods) );
...
void idSessionLocal::SetMainMenuGuiVars( void ) {
	...
	//BC demo build.
	guiMainMenu->SetStateBool( "isdemo", cvarSystem->GetCVarBool("g_demo"));
...
bool idSessionLocal::HandleSaveGameMenuCommand( idCmdArgs &args, int &icmd ) {
	...
	if ( !idStr::Icmp( cmd, "deleteGame" ) )
	{
		...
		if ( choice >= 0 && choice < loadGameList.Num() )
		{
			...
			if ( game->GetSteamInitialized() )
			{
				...
				SteamRemoteStorage()->FileDelete( va("%s/savegames/%s.save", basegame.c_str(), loadGameList[choice].c_str()) );
				SteamRemoteStorage()->FileDelete( va("%s/savegames/%s.tga", basegame.c_str(), loadGameList[choice].c_str()) );
				SteamRemoteStorage()->FileDelete( va("%s/savegames/%s.txt", basegame.c_str(), loadGameList[choice].c_str()) );
			}
#endif
			...
			//BC 4-22-2016 select last item in list after deleting.
			if (loadGameList.Num() > 0)
			{
				guiActive->SetStateString( "loadgame_sel_0", va("%d", loadGameList.Num() - 1) );
			}
	...
	if ( !idStr::Icmp( cmd, "updateSaveGameInfo" ) ) {
		...
		if ( choice >= 0 && choice < loadGameList.Num() ) {
			...
			guiActive->SetStateString( "saveGameTime", time + "z" );
		}
		return true;
	}
...
void idSessionLocal::HandleMainMenuCommands( const char *menuCommand ) {
	...
	for( icmd = 0; icmd < args.Argc(); ) {
		...
		if (game->GetSteamInitialized())
		{
			...
			{
				SteamFriends()->ActivateGameOverlayToWebPage("http://steamcommunity.com/app/240440/workshop");			
			}
		}
#endif
