...
idCVar cm_drawColor(		"cm_drawColor",			"0 1 0 1",	CVAR_GAME,				"color used to draw the collision models" );
...
================
*/
void idCollisionModelManagerLocal::DrawEdge( cm_model_t *model, int edgeNum, const idVec3 &origin, const idMat3 &axis, const idVec4 &linecolor ) {
	int side;
	cm_edge_t *edge;
	...
	if ( edge->internal ) {
		if ( cm_drawInternal.GetBool() ) {
			...
			session->rw->DebugLine( colorGreen, start, end, 1 );
		}
	} else {
		if ( edge->numUsers > 2 ) {
			//session->rw->DebugArrow( colorBlue, start, end, 1 );
			session->rw->DebugLine( colorBlue, start, end, 1 );
		} else {
			session->rw->DebugLine( linecolor, start, end, 1 );
		}
	}
	...
	if ( cm_drawNormals.GetBool() ) {
		...
		session->rw->DebugLine( colorCyan, mid, end, 1 );
	}
}
...
================
*/
void idCollisionModelManagerLocal::DrawPolygon( cm_model_t *model, cm_polygon_t *p, const idVec3 &origin, const idMat3 &axis, const idVec3 &viewOrigin, const idVec4 &linecolor ) {
	int i, edgeNum;
	cm_edge_t *edge;
	...
	if ( cm_drawFilled.GetBool() ) {
		...
		for ( i = 0; i < p->numEdges; i++ ) {
			...
			DrawEdge( model, edgeNum, origin, axis, linecolor );
		}
	}
}
...
void idCollisionModelManagerLocal::DrawNodePolygons( cm_model_t *model, cm_node_t *node,
										   ...
										   const idVec3 &viewOrigin, const float radius, const idVec4 &linecolor ) {
	int i;
	cm_polygon_t *p;
	...
	while (1) {
		for ( pref = node->polygons; pref; pref = pref->next ) {
			...
			DrawPolygon( model, p, origin, axis, viewOrigin, linecolor );
		...
		if ( radius && viewOrigin[node->planeType] > node->planeDist + radius  ) {
			...
			DrawNodePolygons( model, node->children[1], origin, axis, viewOrigin, radius, linecolor );
			node = node->children[0];
		}
	}
}
...
void idCollisionModelManagerLocal::DrawModel( cmHandle_t handle, const idVec3 &modelOrigin, const idMat3 &modelAxis,
	...
	checkCount++;
	DrawNodePolygons( model, model->node, modelOrigin, modelAxis, viewPos, radius, linecolor );
}
