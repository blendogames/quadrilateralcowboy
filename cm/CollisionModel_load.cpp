...
void idCollisionModelManagerLocal::BuildModels( const idMapFile *mapFile ) {
	...
	//BC force collision generation to fix problem where re-named brushes weren't being propogated to new .cm file.
...
bool idCollisionModelManagerLocal::TrmFromModel( const cm_model_t *model, idTraceModel &trm ) {
	...
	{
		common->Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s has too many vertices. Count: %d Max: %d\n", model->name.c_str(), model->numVertices, MAX_TRACEMODEL_VERTS );
		PrintModelInfo( model );
		return false;
	}
	...
	if ( model->numEdges > MAX_TRACEMODEL_EDGES+1 ) {
		common->Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s has too many edges. Count: %d Max: %d\n", model->name.c_str(), model->numEdges, (MAX_TRACEMODEL_EDGES+1) );
		PrintModelInfo( model );
		return false;
	}
