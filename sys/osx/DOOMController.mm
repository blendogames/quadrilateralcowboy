...
#undef strncmp
	...
	// TODO: clean out all the misc. osx arguments the engine doesn't understand.
	if ( false ) {//argc > 1 && strncmp( argv[ 1 ], "-psn", 4 ) ) {
...
const char *Sys_EXEPath( void ) {
	...
	strncpy( exepath, [ [ [ NSBundle mainBundle ] executablePath ] cString ], 1024 );
	return exepath;
}
...
const char *Sys_DefaultSavePath(void) {
	...
	sprintf( savepath, "%s/Library/Application Support/BlendoGames/Quadrilateral Cowboy Demo", [NSHomeDirectory() cString] );
#else
	sprintf( savepath, "%s/Library/Application Support/BlendoGames/Quadrilateral Cowboy", [NSHomeDirectory() cString] );
#endif
...
const char *Sys_DefaultBasePath(void) {
	...
	strncpy( basepath, [ [ [ NSBundle mainBundle ] pathForResource:@"base" ofType:nil ] cString ], 1024 );
